...
//Fog
//Three js. de desteklenen hazır bir fog class ı var
/**In horror movies, they always use fog. The good news is that Three.js supports it already with the Fog class.

The first parameter is the color, the second parameter is the near (how far from the camera does the fog start),
 and the third parameter is the far (how far from the camera will the fog be fully opaque).
...
walls.position.y = 2    // evin yüksekliğinin yarısı kadar olmalı 1.25 sen de ondan çalışmadı sen duvar boyunu 4 belirledin
house.add(walls)
...
roof.position.y = 4 + 1 // DUVARLARIN YÜKSEKLİĞİ + CONE UN YÜKSEKLİĞİNİN YARISINI EKLEDİK
house.add(roof)
...
door.position.z = 2+ 0.001 // 0.01 to get rid of z-figthing
house.add(door)
...
doorLight.position.set(0, 3.8, 2.7)
house.add(doorLight)
